<!DOCTYPE html>
@php($isSuperAdminLogin = request()->is('superadmin/login'))
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>{{ $isSuperAdminLogin ? 'Super Admin' : 'Staff' }} Login - {{ config('app.name', 'Laravel') }}</title>
        ...
        <main class="flex min-h-screen items-center justify-center px-4 py-10">
            <section class="w-full max-w-md rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
                <div class="mb-6 text-center">
                    <h1 class="text-2xl font-semibold">{{ $isSuperAdminLogin ? 'Super Admin' : 'Staff' }} Login</h1>
                    <p class="mt-1 text-sm text-neutral-500">Sign in with your {{ $isSuperAdminLogin ? 'Super Admin' : 'staff' }} account.</p>
                </div>
                ...
                <form method="POST" action="{{ route($loginRouteName, absolute: false) }}" class="space-y-4">
                    ...
                    <div>
                        <label for="staff-email" class="mb-1 block text-sm font-medium text-neutral-700">Username / Email</label>
