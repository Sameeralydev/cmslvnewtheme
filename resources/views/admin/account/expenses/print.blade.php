...
@php
    ...
    $dateFormatted = !empty($bill->date) ? date('d/m/Y', strtotime($bill->date)) : '';
    $payee = $bill->paid_to ?: 'abc';
    $amount = (float)($bill->grand_total ?: 0);
    $amountFormatted = number_format($amount, 0, '.', ',');

    $branchName = $bill->branch_name ?: ($schSetting->name ?? 'Tnt Sol');
    $branchAddress = $schSetting->address ?? 'Gujranwala';
    $branchPhone = $schSetting->phone ?? '923466049180';

    if (!function_exists('convertExpWords')) {
        function convertExpWords($number) {
            $hyphen      = ' ';
            $dictionary  = array(
            ...
            if ($number >= 10000000) {
                ...
                $string .= convertExpWords($crores) . ' Crore ';
                $number %= 10000000;
            }
            if ($number >= 100000) {
                $lakhs = (int)($number / 100000);
                $string .= convertExpWords($lakhs) . ' Lakh ';
                $number %= 100000;
            }
            if ($number >= 1000) {
                $thousands = (int)($number / 1000);
                $string .= convertExpWords($thousands) . ' Thousand ';
                $number %= 1000;
            }
            if ($number >= 100) {
    ...
    $inWords = convertExpWords($amount) . ' Rupees Only';
@endphp
                ...
                <table class="items-tbl">
                    ...
                    <tbody>
                        @forelse($items as $i => $item)
                            <tr>
                                <td>{{ $i + 1 }}</td>
                                <td>{{ $item->head_name ?: ($bill->note ?: 'Expense') }}</td>
                                <td style="text-align: right;">{{ number_format($item->debit_amount ?: $amount, 0, '.', ',') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td>1</td>
                                <td>{{ $bill->head_name ?: ($bill->note ?: 'Expense') }}</td>
