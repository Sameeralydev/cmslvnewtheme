...
@php
    ...
    if (!empty($show_month_count) && $edit_month_count > 0) {
...
<div class="feemaster-container">
    <div class="feemaster-grid">
        {{-- Left Side: Edit Fee Structure Card --}}
        <div>
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">Edit Fee Structure : {{ $current_session_name ?? ($sessionLabel ?? '') }}</h3>
                </div>

                <form id="feemasterform" action="{{ url('admin/account/feemaster/edit/' . $id . '/' . $brc_id) }}" method="POST">
                    @csrf
                    @method('POST')
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <select id="brc_id" name="brc_id" class="form-control-cmsc" onchange="getBranchByID(this.value)">
                                ...
                                @foreach ($branchlist as $brc)
                                    <option value="{{ $brc->id }}" {{ (string)old('brc_id', $feemaster->brc_id) === (string)$brc->id ? 'selected' : '' }}>
                                        {{ $brc->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select id="class_id" name="class_id" class="form-control-cmsc" required>
                                ...
                                @foreach ($classlist as $class)
                                    <option value="{{ $class->id }}" {{ (string)old('class_id', $feemaster->fee_class_id) === (string)$class->id ? 'selected' : '' }}>
                                        {{ $class->class }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select id="frequency" name="frequency" class="form-control-cmsc" required>
                                <option value="">Select</option>
                                <option value="One Time" {{ old('frequency', $feemaster->frequency) === 'One Time' ? 'selected' : '' }}>One Time</option>
                                <option value="Monthly" {{ old('frequency', $feemaster->frequency) === 'Monthly' ? 'selected' : '' }}>Monthly</option>
                                <option value="Yearly" {{ old('frequency', $feemaster->frequency) === 'Yearly' ? 'selected' : '' }}>Yearly</option>
                            </select>
                            @error('frequency')
                                <span style="color: #ff0000; font-size: 12px; display: block; margin-top: 4px;">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group fee-month-count-group" id="monthCountGroup" style="{{ $show_month_count ? '' : 'display:none;' }}">
                            ...
                            <input id="month_count" name="month_count" type="number" min="0" class="form-control-cmsc" value="{{ old('month_count', $edit_month_count) }}" />
                            @error('month_count')
                                <span style="color: #ff0000; font-size: 12px; display: block; margin-top: 4px;">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="feetype_id" name="feetype_id" class="form-control-cmsc" required>
                                ...
                                @foreach ($feetypeList as $feetype)
                                    <option value="{{ $feetype->id }}" {{ (string)old('feetype_id', $feemaster->feetype_id) === (string)$feetype->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <input id="amount_display" type="number" step="any" placeholder="" class="form-control-cmsc" value="{{ old('amount_display', number_format($edit_base_amount, 2, '.', '')) }}" required />
                            <input id="amount" name="amount" type="hidden" value="{{ old('amount', $feemaster->amount) }}" />
                            @error('amount')
                                <span style="color: #ff0000; font-size: 12px; display: block; margin-top: 4px;">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="description" name="description" class="form-control-cmsc" rows="3">{{ old('description', $feemaster->note) }}</textarea>
                            @error('description')
                                <span style="color: #ff0000; font-size: 12px; display: block; margin-top: 4px;">{{ $message }}</span>
                            @enderror
                        </div>
