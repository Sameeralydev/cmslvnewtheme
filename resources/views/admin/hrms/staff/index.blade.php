                    ...
                    <div class="flex">
                        <select name="staff_id" class="h-8 min-w-0 flex-1 border border-[#cfd6e0] bg-[#fbfbfc] px-2 text-[11px] text-[#333] outline-hidden">
                            ...
                            @foreach ($staffOptions as $staffOption)
                                <option value="{{ $staffOption->id }}" @selected($selectedStaffId === (int) $staffOption->id)>
                                    {{ $staffOption->employee_id }} - {{ trim($staffOption->name.' '.$staffOption->surname.' '.$staffOption->father_name) }}
                                </option>
                            @endforeach
                        </select>
                ...
                @forelse ($records as $record)
                    ...
                    <article class="staffinfo-box">
                        <div class="staffleft-box">
                            <img src="{{ $record->image ? asset('uploads/staff_images/'.$record->image) : asset('uploads/staff_images/no_image.png') }}" alt="{{ $fullName }}">
                        </div>
