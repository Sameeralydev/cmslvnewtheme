<?php $__env->startSection('title', 'Academics'); ?>

<?php $__env->startSection('content'); ?>
    <?php echo $__env->make('admin.academics.partials.nav', ['modules' => $modules], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

    <section class="rounded border border-amber-200 bg-amber-50 p-4 text-amber-900">
        <h2 class="font-semibold">Migration status</h2>
        <p class="mt-2 text-sm">
            These Laravel routes and indexes are mapped to confirmed legacy database tables. The original CodeIgniter Academics controllers, models, views, and JavaScript were not present in this workspace, so deeper actions such as calculations, imports, DataTables JSON, paper generation, and marks processing are documented as unresolved until those files are available.
        </p>
    </section>

    <div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <?php $__currentLoopData = $modules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $module): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <a href="<?php echo e(route($module['route'])); ?>" class="rounded border border-neutral-200 bg-white p-4 hover:border-blue-300">
                <p class="text-sm text-neutral-500"><?php echo e($module['table']); ?></p>
                <h2 class="mt-1 font-semibold"><?php echo e($module['label']); ?></h2>
                <p class="mt-2 text-sm text-neutral-600">Legacy URL: /admin/academics/<?php echo e($module['permission']); ?></p>
            </a>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\TNT\cmslv_git_clone2\resources\views\admin\academics\dashboard\index.blade.php ENDPATH**/ ?>