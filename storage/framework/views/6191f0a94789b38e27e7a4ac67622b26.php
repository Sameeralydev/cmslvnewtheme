...
<header class="admin-topbar fixed left-0 right-0 top-0 z-20 h-16 bg-[#24448d] text-white shadow lg:left-[296px]">
    <div class="admin-topbar-inner flex h-full items-center gap-4 px-3">
        <button type="button" class="<?php echo e($isHrmsRoute ? 'text-[20px]' : 'text-2xl'); ?> leading-none" aria-label="Toggle menu">
            <i class="fa-solid fa-bars"></i>
        </button>

        <a
            href="<?php echo e(route('admin.dashboard', absolute: false)); ?>"
            class="admin-brand <?php echo e($isHrmsRoute ? 'min-w-[278px] rounded-l-full rounded-r-[14px] bg-[#4473c9] px-4 text-[18px] tracking-normal' : 'min-w-[300px] rounded-tl-full rounded-tr-2xl bg-[#3f70c9] px-5 text-2xl tracking-wide'); ?> flex h-9 items-center font-bold shadow-inner"
        >
            TNT SOL
        </a>

        <form class="admin-search <?php echo e($isHrmsRoute ? 'max-w-[490px]' : 'max-w-[420px]'); ?> mx-auto hidden w-full items-center overflow-hidden rounded-full bg-white text-neutral-700 shadow-sm md:flex">
...
<?php /**PATH C:\xampp\htdocs\TNT\cmslv_git_clone2\resources\views/admin/partials/header.blade.php ENDPATH**/ ?>