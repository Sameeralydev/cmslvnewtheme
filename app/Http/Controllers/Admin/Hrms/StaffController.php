...
class StaffController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $staffOptions = Staff::query()
            ->where('is_active', 1)
            ->when($selectedBranchId !== null, fn (Builder $query) => $query->where('brc_id', $selectedBranchId))
            ->orderBy('name')
            ->orderBy('surname')
            ->get(['id', 'employee_id', 'name', 'surname', 'father_name']);
    ...
    public function import(Request $request): View|RedirectResponse
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $imported = 0;
            $userId = auth()->id() ?? 0;
            DB::transaction(function () use ($handle, $headers, $validated, $userId, &$imported): void {
                while (($row = fgetcsv($handle)) !== false) {
                    if (count(array_filter($row, fn ($value) => trim((string) $value) !== '')) === 0) continue;
                    $data = array_combine($headers, array_pad($row, count($headers), '')) ?: [];
                    $employeeId = trim((string) ($data['employee_id'] ?? $data['staff_id'] ?? ''));
                    $name = trim((string) ($data['name'] ?? $data['first_name'] ?? ''));
                    $email = trim((string) ($data['email'] ?? $data['email_login_username'] ?? ''));
                    if ($employeeId === '' || $name === '' || $email === '') continue;
                    if (Staff::query()->where('employee_id', $employeeId)->where('brc_id', $validated['brc_id'])->exists() || Staff::query()->where('email', $email)->exists()) continue;
                    Staff::query()->create([
                        'brc_id' => $validated['brc_id'], 'role_id' => $validated['role_id'], 'employee_id' => $employeeId, 'category' => 1,
                        'name' => $name, 'surname' => trim((string) ($data['surname'] ?? $data['last_name'] ?? '')), 'father_name' => trim((string) ($data['father_name'] ?? '')), 'email' => $email,
                        'gender' => trim((string) ($data['gender'] ?? 'Not Specified')), 'dob' => $data['dob'] ?? $data['date_of_birth'] ?? null, 'date_of_joining' => $data['date_of_joining'] ?? now()->toDateString(),
                        'contact_no' => trim((string) ($data['contact_no'] ?? $data['phone'] ?? '')), 'emergency_contact_no' => trim((string) ($data['emergency_contact_no'] ?? '')), 'marital_status' => trim((string) ($data['marital_status'] ?? '')),
                        'local_address' => trim((string) ($data['local_address'] ?? $data['current_address'] ?? '')), 'permanent_address' => trim((string) ($data['permanent_address'] ?? '')), 'note' => trim((string) ($data['note'] ?? '')),
                        'password' => Hash::make((string) random_int(100000, 999999)), 'ch_password' => '', 'is_active' => 1, 'user_id' => $userId, 'created_by' => $userId, 'updated_by' => $userId,
                    ]);
                    $imported++;
                }
                fclose($handle);
            });
            return redirect()->route('admin.hrms.staff.import', ['brc_id' => $validated['brc_id']])->with('success', $imported.' staff record(s) imported successfully.');
        }

        return view('admin.hrms.staff.import', $this->staffFormData($branchId) + ['selectedBranchId' => $branchId]);
    }
    ...
    public function store(?int $branchId, Request $request): RedirectResponse
    {
        ...
        $staff = Staff::query()->create([
            ...
            'note' => $validated['note'] ?? '',
            'image' => $this->storeLegacyUpload($request->file('file'), 'staff_images'),
    ...
    public function update(Request $request, int $staffId): RedirectResponse
    {
        ...
        if ($request->hasFile('file')) {
            $staff->image = $this->storeLegacyUpload($request->file('file'), 'staff_images');
        }
    ...
    private function staffDirectoryQuery(Request $request, ?int $branchId): Builder
    {
        ...
        return Staff::query()
            ...
            ->when($staffId > 0, fn (Builder $query) => $query->where('staff.id', $staffId))
